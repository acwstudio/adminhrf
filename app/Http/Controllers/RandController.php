...
class RandController extends Controller
{
    ...
    {
        $rand = $request->get('rand', 1);
        return $rand < 21 ? HighlightShortResource::collection(Highlight::inRandomOrder()->where('published_at', '<', now())->where('type','=','highlight')->with('images')->limit($rand)->get()) : ['err' => 'nice try bro;)'];
    }

    public function getRandCourses(Request $request)
    {
        $rand = $request->get('rand', 1);
        return $rand < 21 ? CourseShortResource::collection(Highlight::inRandomOrder()->where('published_at', '<', now())->where('type','=','course')->with('images')->limit($rand)->get()) : ['err' => 'nice try bro;)'];
    }

    public function getRandVideoCourses(Request $request)
    {
        $rand = $request->get('rand', 1);
        return $rand < 21 ? CourseShortResource::collection(Highlight::inRandomOrder()->where('published_at', '<', now())->where('type','=','videocourse')->with('images')->limit($rand)->get()) : ['err' => 'nice try bro;)'];
    }

    public function getRandAudioCourses(Request $request)
    {
        $rand = $request->get('rand', 1);
        return $rand < 21 ? CourseShortResource::collection(Highlight::inRandomOrder()->where('published_at', '<', now())->where('type','=','audiocourse')->with('images')->limit($rand)->get()) : ['err' => 'nice try bro;)'];
    }
